...
CHECKPOINT_FILE="../training/checkpoint.2500th.tar"
map_location={"cuda:0":"cuda0"}
DEVICE_IDS=[0,0]
...
for ientry in xrange(start_entry,start_entry+nentries,batchsize_valid):
    ...
    hadc.Draw("COLZ")
    c.Update()
    c.SaveAs("hadc_input.png")

    #cv.imwrite( "cvadc_input.png", source[0,0,:,:].detach().cpu().numpy() )
    ...
    htruey2u.Draw("COLZ")
    c.Update()
    c.SaveAs("hflowy2u_true.png")
    #cv.imwrite( "cvflowy2u_input.png", flow1[0,0,:,:].detach().clamp(-256,256).cpu().numpy() )
    ...
    htruey2v.Draw("COLZ")
    c.Update()
    c.SaveAs("hflowy2v_true.png")
    #cv.imwrite( "cvflowy2v_input.png", flow2[0,0,:,:].detach().clamp(-256,256).cpu().numpy() )
