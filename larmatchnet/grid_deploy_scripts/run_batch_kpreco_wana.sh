...
for ((i=0;i<${STRIDE};i++)); do
    ...
    echo "<<run reco>>"
    RECO_CMD="python $LARFLOW_RECO_DIR/run_keypointreco.py --input-dlmerged ${inputfile} --input-larflow ${kps_pathname} --output ${local_reco_outfile} -rw -tb"
    echo $RECO_CMD
    $RECO_CMD >> ${local_logfile} 2>&1
    ...
    cp ${local_reco_outfile} $OUTPUT_DIR/${subdir}/
