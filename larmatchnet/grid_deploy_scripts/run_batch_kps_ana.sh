...
OUTPUT_DIR=${GRID_SCRIPTS_DIR}/outputdir_ana/${SAMPLE_NAME}
OUTPUT_LOGDIR=${GRID_SCRIPTS_DIR}/logdir_ana/${SAMPLE_NAME}
...
for ((i=0;i<${STRIDE};i++)); do
    ...
    local_outfile=$(echo $baseinput  | sed 's|merged_dlreco|keypointana|g' | sed 's|.root||g' | xargs -I{} echo {}"-${jobname}.root")
    echo "outfile : "$local_outfile
    
    CMD="$LARMATCH_DIR/ana/./keypoint_ana ${inputfile} ${kps_pathname} ${local_outfile}"
    echo $CMD
