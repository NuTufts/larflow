...
class LArMatchSimChHDF5Dataset(Dataset):
    """
    This class is responsible for loading the data needed for
    the semi-supervised pre-training of the backbone network.

    We aim to use Masked, self-supervised tasks to have the net
    learn to encode relationships between the energy depositions points.
    The core of the work is the prediction of the edep values of masked spacepoints
    based on neighboring spacepoints.

    We also include a contrastive learning representation loss requiring
    cosine similarity between like particles and unlike particles.
    """
    ...
    COLUMNS = [
        ...
        "keypoint_truth_pid",
        "wireimage_plane0_coord",
        "wireimage_plane0_feat",
    ...
    COLUMNS_TO_HDF5_NAME = {
        ...
        "spacepoints":"/triplet_data/pos", # (N,3) 3d (reconstructed) position of deposited energy
        "edep":"/triplet_data/edep", # (N,3) spacepoint visible energy deposited in each plane
        "trackid":"/triplet_data/trackid", # (N) geant4 trackids
        "ssnet_label":"/triplet_data/ssnet_label", # (N) class label for particle ID
        ...
        "wireimage_plane0_feat":"/image_data/plane0/feat",   # (NU)
        "wireimage_plane1_coord":"/image_data/plane1/coord", # (NV,2)
        "wireimage_plane1_feat":"/image_data/plane1/feat",   # (NV)
        "wireimage_plane2_coord":"/image_data/plane2/coord", # (NY,2)
        "wireimage_plane2_feat":"/image_data/plane2/feat",   # (NY)
    }
    ...
    def __init__(self, file_paths=None,
        ...
        self.COLS = LArMatchSimChHDF5Dataset.COLUMNS
        ...
        if self.file_paths is not None:
            ...
            for file_path in self.file_paths:
                with h5py.File(file_path, 'r') as hf:
                    # the top level groups are entry_X
                    print(hf.keys())
                    length = len(hf.keys())
    ...
    def prepare_triplet_and_image_arrays_for_network( batchdata, triplet_key="matchtriplet" ):
        ...
        for p in range(3):
            rebatchdata['coord_%d'%(p)] = batchdata[f'wireimage_plane{p}_coord'].astype(np.int64)
            feat_t = np.expand_dims( batchdata[f'wireimage_plane{p}_feat'].astype(np.float32), 1 )
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(self.file_paths[file_idx], 'r') as hf:
            for col in self.COLS:
                ...
                try:
                    hdf_name = f"/entry_{local_idx}"+LArMatchSimChHDF5Dataset.COLUMNS_TO_HDF5_NAME[col]
                    entry_data[col] = np.array(hf[hdf_name])
                except:
                    x = f"/entry_{local_idx}/image_data/plane0"
                    print(x)
                    print(hf[x].keys())
...
def get_data_loader(file_paths, batch_size=2, num_workers=1, shuffle=True,
    ...
    if not load_from_cachefile:
        ...
        if type(file_paths) is str:
            ...
            xpaths = file_paths
        dataset = LArMatchSimChHDF5Dataset(file_paths=xpaths, collate_for_training=collate_for_training,
                                    max_num_spacepoints=max_num_spacepoints,
                                    apply_max_filter=apply_max_filter)
    else:
        dataset = LArMatchSimChHDF5Dataset(load_from_cachefile=file_paths, collate_for_training=collate_for_training, 
                                    max_num_spacepoints=max_num_spacepoints,
                                    apply_max_filter=apply_max_filter)
        
    return DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, collate_fn=LArMatchSimChHDF5Dataset.collate_fn)
