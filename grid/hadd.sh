...
sample="mcc8_bnbcosmic"
#sample="mcc9_extbnb_beta1"
#sample="mcc8v11_bnbcosmic_detsys_cv"

singularity exec ${CONTAINER} bash -c "cd ${WORKDIR_IC} && source /usr/local/root/release/bin/thisroot.sh && source configure.sh && cd grid  && hadd -f output_${sample}.root output/*.root"