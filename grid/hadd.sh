...
singularity exec ${CONTAINER} bash -c "cd ${WORKDIR_IC} && source /usr/local/root/release/bin/thisroot.sh && source configure.sh && cd grid  && hadd -f output_${sample}.root output/truthflow/*.root"