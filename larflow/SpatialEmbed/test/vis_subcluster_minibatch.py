...
def make_figures(entry,plotby="cluster",minprob=0.0):
    ...
    data_dict = voxelloader.getSubclusterTrainingDataBatch(4)
    ...
    if plotby=="q_yplane":
        ...
        for ibatch in range(data_dict["batch_size"]):
            bmask      = data_dict["coord_t"][:,3]==ibatch
            instance_b = data_dict["instance_t"][ bmask ]
            class_b    = data_dict["class_t"][ bmask ]
            coord_b    = data_dict["coord_t"][bmask,:]
            iid_v = np.unique( instance_b ).tolist()
            ninstances = len(iid_v)
            for n,iid in enumerate(iid_v):
                print("== batch [",ibatch,"] - instance [",iid,"] ===")
                idmask = instance_b==iid
                pids = np.unique( class_b[idmask] ).tolist()
                pcounts = {-1:0}
                maxpid = -1
                nmax = 0
                for p in pids:
                    pcounts[p] = (class_b[idmask]==p).sum()
                    if nmax<pcounts[p]:
                        nmax = pcounts[p]
                        maxpid = p
                ...
                print(" pids: ",pids)
                print(" maxpid: ",maxpid," ncounts=",pcounts[p])
                if iid>0:
                ...
                voxtrace = {
                    ...
                    "z":coord_b[idmask,2],
                    "mode":"markers",
                    "name":"[%d,%d]%s"%(ibatch,iid,particle_id_name[maxpid]),
