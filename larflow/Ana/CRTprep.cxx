...
int main( int nargs, char** argv ) {
  ...
  */
  ...
  TFile* outfile = new TFile(Form("prepped_%s",input_th3d_file.c_str()),"recreate");
  ...
  */
  ...
  TTree *tree = new TTree("tree","prepped input tree");
  tree->Branch("row", &row, "row/I");
  tree->Branch("col", &col, "col/I");
  tree->Branch("height", &height, "height/I");
  tree->Branch("chargePerVoxel", &chargePerVoxel, "chargePerVoxel/I");
  ...
  for (int n = 0; n < voxHists; n++) {
    ...
    for (int i = 1; i <= (xyzBins[0]/voxelSize[n]); i++) { // here use i = 1, i <= max, NOT i = 0, i < max (bc bin 0 is underflow in ROOT histograms)
	  ...
	  hitsPerVoxel[n] = h[n]->GetBinContent(i, j, k);
          T[n]->Fill();

	  //hitsPerVoxel[0] = h1->GetBinContent(i, j, k);
	  //T[0]->Fill();
