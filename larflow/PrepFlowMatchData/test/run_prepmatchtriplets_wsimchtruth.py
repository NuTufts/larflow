...
parser.add_argument("-tb","--tick-backward",action='store_true',default=False,help="If true read in legacy tick-backward larcv data [default: False]")
...
if args.tick_backward:
    ...
    io = larcv.IOManager( larcv.IOManager.kREAD, "io", larcv.IOManager.kTickForward )
...
if args.has_mc:
    ...
    ioll.set_data_to_read( larlite.data.kSimChannel,  "driftWC simpleSC" )
    ioll.set_data_to_read( larlite.data.kSimChannel,  "driftWC:simpleSC" )
    ioll.set_data_to_read( larlite.data.kSimChannel,  "largeant" )            
    ioll.open()
...
simchtruthlabeler = larflow.prep.TripletTruthFromSimCh()
...
for ientry in range(start_entry, end_entry+1):
    ...
    for imatchdata in range(preptripletalgo._match_triplet_v.size()):
        tpc_triplet_data = preptripletalgo._match_triplet_v.at(imatchdata)
        if args.has_mc:
            print("process truth labels from SimCh")
            simchtruthlabeler.process_truth_labels( ioll, tpc_triplet_data, "driftWC:simpleSC" )
        
        triplet_v.push_back( tpc_triplet_data )
