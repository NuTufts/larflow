...
parser.add_argument('-k','--input-kpsout',type=str,required=True,help="Input file containing KPSRecoManagerTree")
...
anafile = rt.TFile( args.input_kpsout )
...
kps_nentries = kpsanatree.GetEntries()
nentries = min( (lcv_nentries,ll_nentries,kps_nentries) )
print("LCV ENTRIES: ",lcv_nentries)
print("LL ENTRIES: ",ll_nentries)
print("KPSOUT ENTRIES: ",kps_nentries)
print("NUM ENTRIES: ",nentries)
            

algo = larflow.reco.NuVertexShowerTrunkCheck()
...
for ientry in range(start_entry,nentries):
    ...
    kpsanatree.GetEntry(ientry)
    iolcv.read_entry(ientry)
    for ivtx in range( kpsanatree.nufitted_v.size() ):
        print("======= VERTEX[",ivtx,"]========")
        nuvtx = kpsanatree.nufitted_v.at(ivtx)
        missing_hits = algo.checkNuCandidateProngsForMissingCharge( nuvtx, iolcv, io )
