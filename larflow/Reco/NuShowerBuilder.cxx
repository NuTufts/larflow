                                 ...
                                 std::vector<NuVertexCandidate>& nu_candidate_v )
  {
    ...
    for (auto& nuvtx : nu_candidate_v ) {
      ...
      for ( int iprong=0; iprong<(int)pronginfo_v.size(); iprong++ ) {
        ...
        if ( prong.impactdist<5.0 && prong.nhits>10 ) {
          ...
          for (int jprong=iprong+1; jprong<(int)pronginfo_v.size(); jprong++) {
            ...
            float r = larflow::reco::pointLineDistance3f( prong.trunk_pt, prong.trunk_pt2, pronginfo_v[jprong].trunk_pt );
            float s = larflow::reco::pointRayProjection3f( prong.trunk_pt, prong.trunk_dir, pronginfo_v[jprong].trunk_pt );
            if ( r<20.0 && s>1.0 ) {
