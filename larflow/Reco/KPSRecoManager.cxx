...
namespace reco {
                                ...
                                larlite::storage_manager& ioll )
  {
    ...
    if ( !ev_chstatus or ev_chstatus->ChStatusMap().size()==0 ) {
      // empty channel status: make blanks
      for ( auto& adc : adc_v ) {
	larcv::Image2D blank(adc.meta());
	blank.paint(0.0);
	gapch_v.emplace_back( std::move(adc) );
      }
    }
    else {
      ...
      //                                    5.0, 50, -1.0 );
      gapch_v = _badchmaker.makeOverlayedBadChannelImage( adc_v, *ev_chstatus, 4, 15.0 );
                                        ...
                                        larlite::storage_manager& ioll )
  {
    ...
    _splithits_wcfilter.set_verbosity( larcv::msg::kDEBUG );
