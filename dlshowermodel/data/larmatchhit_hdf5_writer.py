...
class LArMatchHitHDF5Writer:
    ...
    def make_batch_sparse_tensors( batchdata, device=None, verbose=False, triplet_key="matchtriplet" ):
        ...
        try:
            ...
            print("could not load pytorch or numpy")
            sys.exit(0)
        ...
        matchtriplet_v = []
        for b,data in enumerate(batchdata):
            matchtriplet_v.append( torch.from_numpy(data["matchtriplet"]).to(device) )
            if verbose:
