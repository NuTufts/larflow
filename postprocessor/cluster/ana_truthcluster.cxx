...
int main( int nargs, char** argv ) {
  ...
  io_larlite.add_in_filename( input_cluster );
  ...
  for (int ientry=0; ientry<nentries; ientry++) {
    ...
    // get hits
    auto ev_larflowclusters = (larlite::event_larflowcluster*)io_larlite.get_data(larlite::data::kLArFlowCluster,"flowtruthclusters");
    ...
    int iclust=0;
    int nclusters = ev_larflowclusters->size();
    for ( auto const& cluster_v: *ev_larflowclusters ) {
      if ( iclust+1==nclusters ) break; // skip last cluster which is left overs
      
      for ( auto const& hit : cluster_v ) {
	...
	if ( pixval>threshold ) {
	  ...
	  // if has marker, it was a part of a cluster
