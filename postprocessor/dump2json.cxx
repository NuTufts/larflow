...
InputArgs_t parseArgs( int nargs, char** argv ) {
  ...
  commands.push_back( Arg_t("-mc","input: mcinfo [required]") );
  ...
  for ( int iarg=1; iarg<nargs; iarg++ ) {
	...
	if ( command.flag=="-i" ) {
	  argconfig.input_larflowhits = argv[iarg+1];
	  argconfig.hasinput = true;
	}
	else if ( command.flag=="-mc" ) {
	  argconfig.input_mcinfo = argv[iarg+1];
	  argconfig.hasmc = true;
	}
	else if ( command.flag=="-o" ) {
	  argconfig.output_json = argv[iarg+1];
	  argconfig.outputset = true;
	}
	// operation complete
	break;
  ...
  // Configuration Check
  // -------------------
  if ( !argconfig.hasinput || !argconfig.outputset || !argconfig.hasmc ) {
